...
//
// Q: Does a MutationObserver have a strong reference to the observed node or only a weak reference?
// A: It has only weak references to observed nodes; see https://github.com/whatwg/dom/issues/1159
//
...
export class ElementTypeProcessor {
    ...
    #elementType_lowercase;
    #elementType_uppercase;
    #processElement;
    #started = false;
    ...
    constructor(elementType, callback) {
        this.#elementType_lowercase = elementType.toLowerCase();
        this.#elementType_uppercase = elementType.toUpperCase();
    ...
    scanSubtree(root) {
        if (root.matches(this.#elementType_lowercase)) this.handleMatch(root);
        root.querySelectorAll(this.#elementType_lowercase).forEach((match) => this.handleMatch(match));
    }
